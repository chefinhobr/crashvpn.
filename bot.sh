...
#	DESENVOLVIDO POR:	Red YT 
#   API SHELLBOT:   SHAMAN
#	CONTATO TELEGRAM:	http://t.me/redytvpn Family_vpn
#	CANAL TELEGRAM:	http://t.me/redssh_oficial
#===================================================
[[ ! -d /etc/redyt ]] && exit 0
[[ ! -d /etc/bot ]] && exit 0
source ShellBot.sh
...
[[ -z $api_bot ]] && exit 0
[[ -z $id_admin ]] && exit 0
[[ $(awk -F" " '{print $2}' /usr/lib/licence) != "@redssh_oficial" ]] && exit 0
...
fun_menu() {
    if [[ "${message_from_id[$id]}" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO(a) AO BOT REDSSH  </b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
        ...
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO(a) AO BOT redyt</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
        env_msg+="⚠️ <i>SELECIONE UMA OPCAO ABAIXO !</i>\n\n"
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} --text "$env_msg" \
...
fun_ajuda() {
    ...
    if [[ "$id_chatuser" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>BEM VINDO(a) AO BOT REDSSH</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
criar_user() {
    ...
    echo "$senha" >/etc/redyt/senha/$usuario
...
fun_del_user() {
    ...
    [[ "${message_from_id[$id]}" = "$id_admin" ]] && {
        ...
        rm /etc/redyt/senha/$usuario >/dev/null 2>&1
    } || {
    ...
    [[ -e /etc/redyt/userteste/$usuario.sh ]] && rm /etc/redyt/userteste/$usuario.sh
    ...
    if [[ "${callback_query_from_id[$id]}" = "$id_admin" ]]; then
        ...
        fun_infu() {
            ...
            for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
                info='===========================\n'
                [[ -e /etc/redyt/senha/$user ]] && senha=$(cat /etc/redyt/senha/$user) || senha='Null'
...
sobremim() {
    ...
    msg+="<b>🤖 BOT REDSSH 🤖</b>\n"
    msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    msg+="<b>Desenvolvido por:</b> @redytvpn\n"
    msg+="<b>Canal Oficial:</b> @redssh_oficial\n\n"
...
fun_teste() {
    ...
    cat <<-EOF >/etc/redyt/userteste/$usuario.sh
	#!/bin/bash
	# USUARIO TESTE
	...
	rm /etc/redyt/senha/$usuario > /dev/null 2>&1
	rm /etc/redyt/userteste/$usuario.sh
	EOF
    chmod +x /etc/redyt/userteste/$usuario.sh
    echo "/etc/redyt/userteste/$usuario.sh" | at now + $t_time hour >/dev/null 2>&1
...
fun_exp_user() {
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
            ...
            [[ -e /etc/redyt/userteste/$user.sh ]] && rm /etc/redyt/userteste/$user.sh
...
fun_backauto() {
    [[ "${callback_query_from_id[$id]}" = "$id_admin" ]] && {
        [[ ! -d /etc/redyt/backups ]] && {
            ...
            [[ -d /etc/redyt/backups ]] && rm -rf /etc/redyt/backups
            ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
...
backup_auto() {
    ShellBot.sendDocument --chat_id $id_admin \
        --document "@/etc/redyt/backups/backup.vps" \
        --caption "$(echo -e "♻️ BACKUP AUTOMATICO ♻️")"
    rm /etc/redyt/backups/backup.vps
    return 0
}
...
while :; do
    [[ -e "/etc/redyt/backups/backup.vps" ]] && {
        backup_auto
    }
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '👥 ADICIONAR REVENDEDOR 👥\n\nInforme o nome:')
                    ...
                    echo "Nome: ${message_text[$id]}" >$CAD_ARQ
                    ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
                        --text 'Informe o user dele [Ex: @redytvpn]:' \
                        --reply_markup "$(ShellBot.ForceReply)"
                    ;;
                'Informe o user dele [Ex: @redytvpn]:')
                    verifica_acesso
                    [[ "$_erro" == '1' ]] && break
                    _VAR1=$(echo -e ${message_text[$id]} | awk -F '@' {'print $2'})
                    [[ -z $_VAR1 ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "❌ Erro \n\n⚠️ Informe o user [EX: @redytvpn]")" \
                            --parse_mode html
                        break
                    }
